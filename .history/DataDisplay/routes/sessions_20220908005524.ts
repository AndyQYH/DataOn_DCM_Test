...
router.post('/', userAuth, async(req:Request, res:Response) => {
    ...
    if(req.body.username){
        ...
        const row = await db_users.get(`SELECT * FROM ${table} WHERE user = ? AND username = ? AND password = ?`, [req.params.id,username_disabled.split(':')[0],password])
        ...
        if (!row) {
            //response validate data to register.ejs
                ...
                res.cookie(username_disabled,req.params.id,  {
                    httpOnly: true,
                    maxAge: maxAge * 1000, // 3hrs in ms
                })
