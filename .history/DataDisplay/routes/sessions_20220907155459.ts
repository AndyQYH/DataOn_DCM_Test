...
router.post('/', userAuth, async(req:Request, res:Response) => {
    ...
    if(req.body.username){
        ...
        const row = await db_users.get(`SELECT * FROM ${table} WHERE user = ? AND username = ? AND password = ?`, [user,username,password])
        
        
        if (row) {
        //response validate data to register.ejs
            console.log('has errors!')
            res.status(400).send('Issues encountered when creating session... Try different username/password...')
        }else{
            console.log('successfully created session ...')
            var maxAge = 3 * 60 * 1000

            db_users.insertDB(table,[user,username,password,device,date])

            res.cookie(user, username + ':' + date, {
                httpOnly: true,
